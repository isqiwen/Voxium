...
namespace Generator
{
    ...
    int SerializerGenerator::generate(std::string path, SchemaModule schema)
    {
        ...
        include_headfiles.push_back(
            Mustache::data("headfile_name", Utils::makeRelativePath(m_root_path, path).generic_string()));
                ...
                if (!include_file.empty())
                {
                    ...
                    if (file_path != include_file_base)
                    {
                        include_headfiles.push_back(Mustache::data(
                            "headfile_name", Utils::makeRelativePath(m_root_path, include_file_base).generic_string()));
                    }
                }
            }
                ...
                if (field->m_type.find("std::vector") == 0)
                {
                    ...
                    if (!include_file.empty())
                    {
                        ...
                        if (file_path != include_file_base)
                        {
                            include_headfiles.push_back(Mustache::data(
                                "headfile_name",
                                Utils::makeRelativePath(m_root_path, include_file_base).generic_string()));
                        }
                    }
                }
                // deal normal
        ...
        m_include_headfiles.push_back(
            Mustache::data("headfile_name", Utils::makeRelativePath(m_root_path, file_path).generic_string()));
        return 0;
    }
